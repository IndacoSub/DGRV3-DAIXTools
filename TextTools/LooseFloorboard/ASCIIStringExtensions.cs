...
public static class StringExtensions
{
    ...
    public static string FoldToASCII(this string input, bool no_z, int? length = null)
    {
        int end = length ?? input.Length;

        string fn = Path.GetFileName(input);
        
        var foldedString = new StringBuilder();

        const bool no_fullwidth = false;
        const bool use_weird_characters_from_font = false;

        char last = '\0';
        bool is_accent = false;
        bool is_vowel = false;
        bool is_macron = false;
        bool is_fullwidth_jp = false;
        for (int pos = 0; pos < end; ++pos)
        {
            ...
            if (c < '\u0080')
            {
                if (c != last && last >= '\u0080' && is_vowel)
                {
                    if(!is_macron && !is_fullwidth_jp && is_accent && !use_weird_characters_from_font)
                    {
                        foldedString.Append('\'');
                    }
                }

                // Workaround for font(s) not supporting the Z letter
                if(c == 'z' && no_z)
                {
            ...
            else
            {
                ...
                // Remove fullwidth?
                if(!no_fullwidth && is_fullwidth_jp)
                {
                    // c = input[pos]
                    foldedString[foldedString.Length - 1] = c;
                }

                if (use_weird_characters_from_font && is_accent && use_weird_characters_from_font)
                {
                    char nc = foldedString[foldedString.Length - 1];
                    switch(nc)
                    {
                        case 'a':
                            nc = '\u4E0A';  //U + 4E0A; // 上
                            break;
                        case 'e': // (multiple accents)
                            switch(c)
                            {
                                case '\u00E8': // "è"
                                    nc = '\u4E0B';  //U + 4E0B; // 下
                    ...
                    foldedString[foldedString.Length - 1] = nc;
                }
                ...
                if (use_weird_characters_from_font && is_macron && !is_accent && use_weird_characters_from_font)
                {
                    char nc = foldedString[foldedString.Length - 1];
                    switch (nc)
                    {
                ...
                // THIS IS JUST IN CASE THERE'S SOMETHING AT THE END
                if (pos == end - 1 && foldedString[foldedString.Length - 1] != '\'' && is_vowel)
                {
                    if(!is_macron && !is_fullwidth_jp && is_accent && !use_weird_characters_from_font)
                    {
                        foldedString.Append('\'');
