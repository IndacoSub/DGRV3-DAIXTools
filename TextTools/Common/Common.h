...
// VAR_X, meaning
using VarEntry = std::pair<std::string, std::string>;
...
namespace Common {
	...
	inline void DoLog(std::string const& Message, const std::source_location Where, std::string const& Project) {
		...
		std::ofstream logout("texttools_log.txt", std::ios::out | std::ios::app);
		logout << ss.str();
		logout.close();
	}
#else
	...
	inline int executeBatch(const char* fullBatchFileName, std::vector<std::string> const& arguments = {}, bool silent = false) {
		...
		if (!silent) {
			LOG("\n", HERE, "Common");
			LOG("[TextTools] Executing command: " + batch, HERE, "Common");
			LOG("\n", HERE, "Common");
		}
		...
		// Could maybe break if oss contains spaces
	...
	inline std::string ShortenFilename(std::string const& filename, int slashes) {
		...
		// No example of what this does?
		// Builds a shortened path by keeping only the last `slashes + 1` components.
		// Example: ShortenFilename("a/b/c/d/e.txt", 2) → "c/d/e.txt"
